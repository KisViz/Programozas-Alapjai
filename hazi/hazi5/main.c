...
/*void tombFordit (LEGO_lap tomb[], int hossz) {

    LEGO_lap tart;
    for (int i = 0, j = hossz - 1; i < hossz/2; ++i, --j) {
        tart = tomb[i];
        tomb[i] = tomb[j];
        tomb[j] = tart;
    }
...
void rendez(LEGO_lap LEGO_lapok[], int n) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if ( (LEGO_lapok[j].a_oldal * LEGO_lapok[j].b_oldal) > (LEGO_lapok[i].a_oldal * LEGO_lapok[i].b_oldal) ) {
                tart = LEGO_lapok[i];
                LEGO_lapok[i] = LEGO_lapok[j];
                LEGO_lapok[j] = tart;
            }
        }
    }

    //tombFordit(LEGO_lapok, n);
}
